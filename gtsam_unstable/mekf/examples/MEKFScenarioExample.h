 ...
 * @brief Shared helpers for baseline-MEKF IMU-only trajectory examples.
 *
 * IMU-only mode: the filter dead-reckons with propagate() alone (no position
 * updates). Mirrors tfg_inekf/examples/TFGInEKFScenarioExample.h (same
 * RunOptions, defaultQc(), CSV schema, runScenario flow) so the MEKF and the
 * geometric filters can be driven through identical scenarios and compared with
 * the same NEES/consistency scripts.
 *
...
using Cov15 = MultiplicativeEKF::Covariance;

struct RunOptions {
  double duration = 30.0;
  double dt = 0.01;
  std::string output_path = "mekf_trajectory.csv";
...
inline RunSummary runScenario(const gtsam::Scenario& scenario,
  ...
  MultiplicativeEKF filter(X0, P0);
