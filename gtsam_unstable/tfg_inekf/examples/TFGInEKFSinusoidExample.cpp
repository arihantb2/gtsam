/**
 * @file  TFGInEKFSinusoidExample.cpp
 * @brief TFG-IEKF IMU-only, sinusoidal (weaving) trajectory.
 *
 ...
 * gives exact ground truth and IMU at every t. The time-varying specific force
 * exercises the predict error dynamics periodically.
 *
...
int main(int argc, char* argv[]) {
  ...
  try {
    const RunOptions opts =
        parseRunOptions(argc, argv, "tfg_inekf_sinusoid.csv");
    ...
    std::cerr << "TFGInEKFSinusoidExample failed: " << e.what() << '\n';
    return 2;
  }
  return 0;
}
