...
TGState TGState::identity() {
  ...
  xi.b_omega = Eigen::Vector3d::Zero();
...
tgeqf::TGState traits<tgeqf::TGState>::Retract(const tgeqf::TGState& xi,
  ...
  result.p       = xi.p + delta.segment<3>(3);
  result.v       = xi.v + delta.segment<3>(6);
  result.b_omega = xi.b_omega + delta.segment<3>(9);
  result.b_v     = xi.b_v     + delta.segment<3>(12);
  result.b_a     = xi.b_a     + delta.segment<3>(15);
  return result;
}
...
traits<tgeqf::TGState>::TangentVector traits<tgeqf::TGState>::Local(
  ...
  delta.segment<3>(3)  = xi_est.p       - xi_ref.p;
  delta.segment<3>(6)  = xi_est.v       - xi_ref.v;
  delta.segment<3>(9)  = xi_est.b_omega - xi_ref.b_omega;
  delta.segment<3>(12) = xi_est.b_v     - xi_ref.b_v;
  delta.segment<3>(15) = xi_est.b_a     - xi_ref.b_a;
  return delta;
}
...
void traits<tgeqf::TGState>::Print(const tgeqf::TGState& xi,
  ...
  std::cout << str;
  xi.R.print("  R: ");
  std::cout << "  p: "       << xi.p.transpose()       << "\n"
            << "  v: "       << xi.v.transpose()       << "\n"
            << "  b_omega: " << xi.b_omega.transpose() << "\n"
